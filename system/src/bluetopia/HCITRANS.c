...
typedef struct _tagUartContext_t
{
   ...
   UART_HandleTypeDef huart_bluetooth; //the uart handle for the bluetooth module. USART2 Added by Tim
   /* Default UART config.                                              */
   ...
   //CTS pin PA0. HCITR_CTS_IRQ_LINE = EXTI_Line0
...
static void ConfigureGPIO(GPIO_TypeDef *Port, unsigned int Pin, uint32_t Mode);
...
//   GpioConfiguration.GPIO_PuPd  = GPIO_PuPd_NOPULL;
   ...
   GpioConfiguration.Speed = GPIO_SPEED_FREQ_MEDIUM;
...
static void SetSuspendGPIO(Boolean_t Suspend)
{
   ...
   if(Suspend)
   {
      ...
      //ConfigureGPIO(HCITR_CTS_GPIO_PORT, HCITR_CTS_PIN, GPIO_Mode_IN);
	   gpioInitStruct.Pin = HCITR_CTS_PIN;
	   ...
	   HAL_GPIO_Init(HCITR_CTS_GPIO_PORT, &gpioInitStruct);
      ...
      //NVIC_EnableIRQ(HCITR_CTS_IRQ);
	   ...
	   //Configure external interrupt on CTS pin PA0.
	   //for handling when the USB micro is ready to receive data
	   //must be both rising and falling edge because USB MCU gives rising edge
	   //when ready for new transfer, and falling edge when ready for bulk data
	   SYSCFG->EXTICR[0] = (SYSCFG->EXTICR[0] & ~SYSCFG_EXTICR1_EXTI0) | SYSCFG_EXTICR1_EXTI0_PA; //clear the EXT0 bits, set to PA0
	   EXTI->FTSR1 |= EXTI_FTSR1_FT0; //enable falling edge interrupt on external pins 0
	   //EXTI->RTSR1 |= EXTI_RTSR1_RT10; //enable rising edge interrupt on external pins 0
	   EXTI->IMR1 |= EXTI_IMR1_IM0; //unmask the interrupt request
	   NVIC_SetPriority(HCITR_CTS_IRQ, INTERRUPT_PRIORITY); //high priority, will get handled in time.
	   NVIC_EnableIRQ(HCITR_CTS_IRQ); //not enabling the interrupt yet because it will conflict with ICC start up
	                                             //will wait until after the USB micro has signaled it has initialized

      /* Set RTS as an output and pull high.                            */
      //ConfigureGPIO(HCITR_RTS_GPIO_PORT, HCITR_RTS_PIN, GPIO_Mode_OUT);
      gpioInitStruct.Pin = HCITR_RTS_PIN;
      gpioInitStruct.Mode = GPIO_MODE_OUTPUT_PP;
      gpioInitStruct.Pull = GPIO_NOPULL;// changed from GPIO_PULLUP
      gpioInitStruct.Speed = GPIO_SPEED_FREQ_MEDIUM; //5-25 Mhz
      gpioInitStruct.Alternate = GPIO_AF7_USART2;
      HAL_GPIO_Init(HCITR_RTS_GPIO_PORT, &gpioInitStruct);
...
static void TxInterrupt(void)
{
   ...
   //while((UartContext.TxBytesFree != OUTPUT_BUFFER_SIZE) && (HCITR_UART_BASE->SR & USART_FLAG_TXE))
	while((UartContext.TxBytesFree != OUTPUT_BUFFER_SIZE) && (HCITR_UART_BASE->ISR & USART_FLAG_TXE))
	{
...
{  //changed by tim
   ...
   //while((UartContext.RxBytesFree) && (HCITR_UART_BASE->SR & (USART_FLAG_RXNE | USART_FLAG_ORE)))
	while((UartContext.RxBytesFree) && (HCITR_UART_BASE->ISR & (USART_FLAG_RXNE | USART_FLAG_ORE)))
	{
      //if(HCITR_UART_BASE->SR & USART_FLAG_ORE)
	  if(HCITR_UART_BASE->ISR & USART_FLAG_ORE)
...
void HCITR_CTS_IRQ_HANDLER(void)
{
   ...
   EXTI_ClearFlag(HCITR_CTS_IRQ_LINE);
}
...
int BTPSAPI HCITR_COMOpen(HCI_COMMDriverInformation_t *COMMDriverInformation, HCITR_COMDataCallback_t COMDataCallback, unsigned long CallbackParameter)
{
   ...
   GPIO_InitTypeDef gpioInitStruct;//added to initializes gpio pins on stm32l4xx
   ...
   if((!HCITransportOpen) && (COMMDriverInformation) && (COMDataCallback))
   {
      ...
      __USART2_CLK_ENABLE(); //enable uart2 clock for bluetooth uart
      __GPIOA_CLK_ENABLE(); //enable the clock for port A used by bluetooth uart tx, rx, cts, rts pins
      __GPIOD_CLK_ENABLE(); //enable the clock for port D used by bluetooth uart reset pin

      /* Configure the GPIO.                                            */
      //ConfigureGPIO(HCITR_RESET_GPIO_PORT, HCITR_RESET_PIN, GPIO_Mode_OUT);
      gpioInitStruct.Pin = HCITR_RESET_PIN;
      gpioInitStruct.Mode = GPIO_MODE_OUTPUT_PP;
      gpioInitStruct.Pull = GPIO_NOPULL;// changed from GPIO_PULLUP
      gpioInitStruct.Speed = GPIO_SPEED_FREQ_MEDIUM; //5-25 Mhz
      HAL_GPIO_Init(HCITR_RESET_GPIO_PORT, &gpioInitStruct);

      SetReset();

      //ConfigureGPIO(HCITR_TXD_GPIO_PORT, HCITR_TXD_PIN, GPIO_MODE_AF_PP);
      //GPIO_PinAFConfig(HCITR_TXD_GPIO_PORT, HCITR_TXD_PIN, HCITR_UART_GPIO_AF);
      gpioInitStruct.Pin = HCITR_TXD_PIN;
      gpioInitStruct.Mode = GPIO_MODE_AF_PP;
      gpioInitStruct.Pull = GPIO_NOPULL;// changed from GPIO_PULLUP
      gpioInitStruct.Speed = GPIO_SPEED_FREQ_MEDIUM; //5-25 Mhz
      gpioInitStruct.Alternate = GPIO_AF7_USART2;
      HAL_GPIO_Init(HCITR_TXD_GPIO_PORT, &gpioInitStruct);

      //ConfigureGPIO(HCITR_RXD_GPIO_PORT, HCITR_RXD_PIN, GPIO_MODE_AF_PP);
      //GPIO_PinAFConfig(HCITR_RXD_GPIO_PORT, HCITR_RXD_PIN, HCITR_UART_GPIO_AF);
      gpioInitStruct.Pin = HCITR_RXD_PIN;
      gpioInitStruct.Mode = GPIO_MODE_AF_PP;
      gpioInitStruct.Pull = GPIO_NOPULL;// changed from GPIO_PULLUP
      gpioInitStruct.Speed = GPIO_SPEED_FREQ_MEDIUM; //5-25 Mhz
      gpioInitStruct.Alternate = GPIO_AF7_USART2;
      HAL_GPIO_Init(HCITR_RXD_GPIO_PORT, &gpioInitStruct);
     ...
     //GPIO_PinAFConfig(HCITR_CTS_GPIO_PORT, HCITR_CTS_PIN, HCITR_UART_GPIO_AF);
      ...
      huart_bluetooth.Instance = USART2;
      huart_bluetooth.Init.BaudRate = COMMDriverInformation->BaudRate;
      ...
      huart_bluetooth.Init.HwFlowCtl = UART_HWCONTROL_RTS_CTS;
      huart_bluetooth.Init.OverSampling = UART_OVERSAMPLING_16;
      huart_bluetooth.Init.OneBitSampling = UART_ONEBIT_SAMPLING_DISABLED;
      huart_bluetooth.AdvancedInit.AdvFeatureInit = UART_ADVFEATURE_NO_INIT;
      HAL_UART_Init(&huart_bluetooth);
      ...
      SYSCFG->EXTICR[0] = (SYSCFG->EXTICR[0] & ~SYSCFG_EXTICR1_EXTI0) | SYSCFG_EXTICR1_EXTI0_PA; //clear the EXT0 bits, set to PA0
      EXTI->FTSR1 |= EXTI_FTSR1_FT0; //enable falling edge interrupt on external pins 0
      //EXTI->RTSR1 |= EXTI_RTSR1_RT0; //enable rising edge interrupt on external pins 0
      EXTI->IMR1 |= EXTI_IMR1_IM0; //unmask the interrupt request
      NVIC_SetPriority(HCITR_CTS_IRQ, INTERRUPT_PRIORITY); //high priority, will get handled in time.
      NVIC_EnableIRQ(HCITR_CTS_IRQ);

      //RCC_APB2PeriphClockCmd(RCC_APB2Periph_SYSCFG, DISABLE);
      RCC_APB2PeriphClockCmd(RCC_APB2ENR_SYSCFGEN, DISABLE);
...
#if (defined(SUPPORT_TRANSPORT_SUSPEND) || defined(USE_SOFTWARE_CTS_RTS))
      //commented out below lines by tim
