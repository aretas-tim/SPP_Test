...
void BTPSAPI BTPS_Init(void *UserParam)
{
   //RCC_ClocksTypeDef RCC_Clocks;
	uint32_t sysClockFreq; //added by Tim becase L4 family has no RCC_ClocksTypeDef type
   ...
   sysClockFreq = HAL_RCC_GetSysClockFreq(); //get the system clock frequency. added by tim

   //SysTick_Config((RCC_Clocks.SYSCLK_Frequency * TICKS_PER_TIMER_INTERRUPT) / 1000);
   SysTick_Config((sysClockFreq * TICKS_PER_TIMER_INTERRUPT) / 1000);
